...
import { Check, ChevronDown, CircleAlert, ExternalLink, LocateFixed, MapPin, RotateCcw, Search, ShieldCheck, Sparkles, X } from 'lucide-react';
...
export function App() {
  ...
  useEffect(()=>{const close=(event:KeyboardEvent)=>{if(event.key==='Escape')setProfileOpen(false)};window.addEventListener('keydown',close);return()=>window.removeEventListener('keydown',close)},[]);
  const chooseProvider=(npi:string)=>{setSelectedNpi(npi);setProfileOpen(true)};
...
function ProviderDrawer({provider:p,photo,onClose}:{provider:RankedProvider;photo?:ProviderPhoto;onClose:()=>void}){
  ...
  return <div className="drawer-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><aside className="provider-drawer" role="dialog" aria-modal="true" aria-label={`${name(p)} public profile`}>
    ...
    <div className="drawer-metrics"><div><b>{p.score}</b><span>Priority score</span></div><div><b>{p.cityTrials.length}</b><span>Recruiting trials</span></div><div><b>{p.exactFit}%</b><span>Specialty fit</span></div></div>
    ...
    <section className="drawer-section"><span>Why this provider</span><p>Active oncology NPI with {p.cityTrials.length} recruiting cancer {p.cityTrials.length===1?'trial':'trials'} in the same market. Priority combines specialty fit, market trial activity, and NPI record recency.</p><div className="score-bars"><ScoreBar label="Specialty fit" value={p.exactFit}/><ScoreBar label="Trial activity" value={p.trialSignal}/><ScoreBar label="Record recency" value={p.recency}/></div></section>
    <div className="drawer-sources"><div><ShieldCheck/><span><b>Source-backed</b><small>NPI updated {p.basic.last_updated??'date unavailable'}</small></span></div><a href={`${NPI_SOURCE}provider-view/${p.number}`} target="_blank" rel="noreferrer">Open NPI record <ExternalLink/></a><a href="https://clinicaltrials.gov/" target="_blank" rel="noreferrer">Open trial source <ExternalLink/></a></div>
  </aside></div>
}
