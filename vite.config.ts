...
export default defineConfig(({mode}) => {
  ...
  /* Lightning CSS treats `-webkit-backdrop-filter` as satisfying the same
     property as the unprefixed form and keeps only one of them. It kept the
     prefixed one, which Chrome and Safari honour and Firefox ignores outright,
     so every frosted surface, the map veil included, rendered flat in Firefox
     on the built site while dev served the unminified rule and looked right.
     Setting cssTarget alone does not change that choice, so the CSS is minified
     with esbuild, which leaves both declarations in place. */
  build: {
